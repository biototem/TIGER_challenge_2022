    ...
    def resize(self,resize_shape_w,resize_shape_h,interpolation = 'default'):
        if interpolation == 'default':
            if self._im.shape[1] < 100 and self._im.shape[0] < 100:
                im_resize = cv2.resize(self._im,(round(resize_shape_w),round(resize_shape_h)), interpolation=cv2.INTER_NEAREST)
            elif resize_shape_w < self._im.shape[1] and resize_shape_h < self._im.shape[0]:
                im_resize = cv2.resize(self._im,(round(resize_shape_w),round(resize_shape_h)), interpolation=cv2.INTER_AREA)
            else:
                im_resize = cv2.resize(self._im,(round(resize_shape_w),round(resize_shape_h)), interpolation=cv2.INTER_CUBIC)
        else:
            ...
            im_resize = cv2.resize(self._im, (round(resize_shape_w), round(resize_shape_h)), interpolation=interpolation)
        return im_resize