...
def visual(root: str):
    ...
    for model_name in model_names:
        ...
        for metric in metrics:
            if metric not in ['wdice']:
                continue
            idx += 1
            plt.subplot(4, 5, idx)
            plt.title(f'{model_name.replace("auto-save-epoch-", "3-0-")}-{metric}')
            # 计算组评分
            for group, color in zip(['train', 'valid', 'test'], ['g', 'y', 'r']):
                # 获取分组信息
                keys = list(filter(lambda key: key_set[key] == group, data_keys[:-3]))
                assert len(keys) >= 2, f'分组数量少于 2 是不可能的，请检查这里的 bug：{keys}'
                # 获得图片评分
                scores = score_table[model_name, keys].data.tolist()
                scores.sort(reverse=True)
                score = score_table[model_name, group]
