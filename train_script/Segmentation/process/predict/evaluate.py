...
def do_evaluate(model_name: str, root: str):
    # 加载统计信息
    data_table = Table.load(path=join(root, model_name, 'calculate.txt'), dtype=np.int64)
    # 加载分组信息
    ...
    # 以下开始进入评测流程
    score_table = Table(
        data_keys + ['train', 'valid'], metrics,
        k_v_sep=data_table.k_v_sep, key_sep=data_table.key_sep,
    )
    # 计算单图评分
    for data_key in data_keys:
        # 开始打分
        dice, dice26, dice134, ms, ws = evaluate_dice(matrix_table=data_table[data_key])
        if sum(ms) <= 1: continue
    ...
    for group in ['train', 'valid']:
        ...
        matrix = data_table[group_keys].data.sum(axis=0)
        # 将统计矩阵还原为统计表
        matrix_table = Table(
            CLASS_NAMES,
            CLASS_NAMES,
        ...
        # 存储组评分 -> 组评分无需过滤死线数据
        score_table[group, 'wdice'] = sum([d * g for d, g in zip(dice, ws)]) / sum(ws)
        score_table[group, 'weight'] = ws
        score_table[group, 'mdice'] = sum([d * g for d, g in zip(dice, ms)]) / sum(ms)
        score_table[group, 'dice'] = dice
        score_table[group, 'score'] = (dice[1] + dice26) / 2
        score_table[group, 'dice1'] = dice[1]
        score_table[group, 'dice26'] = dice26
        score_table[group, 'dice134'] = dice134
    ...
    # 保存评估结果
    score_table.save(path=join(root, model_name, 'evaluate.txt'))
